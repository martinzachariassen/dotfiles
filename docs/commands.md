...
| `chezdistill` | **`claudeDistiller` only.** Distil this machine's Claude Code transcripts into `~/Documents/TheArchive/30-Claude` and render the `MAIN.md` the global persona `@`-imports, so yesterday's conclusions are in context tomorrow. launchd runs it at 01:00 and `--weekly` on Sunday at 02:00; both fire on **wake** when the Mac slept through, coalescing missed firings into one run — which is why the job tracks a cursor rather than "yesterday" and so cannot lose a night. **It creates nothing:** preflight requires the vault, its `.obsidian` dir and `30-Claude/` to already exist and exits 0 otherwise, because an uncloned vault would otherwise look like an empty directory and collect reports in a dead end. Both Macs contribute — each writes only its own `.state/extracts/<date>/<hostname>.json`, so they cannot conflict, and whichever runs later re-renders the day from everyone's extracts; a `## Sources` fingerprint makes a repeat run a no-op with no model call. Nothing reaches `MAIN.md` until it has been seen in **two** distinct sessions, so one misreading can't become a global rule; each report opens with `## MAIN.md changes`. `--setup` is the one exception to "creates nothing", and the way to turn the whole thing on: it verifies the vault and its `.obsidian` dir (never creating either), creates `30-Claude/` behind a confirm, adds `claudeDistiller` to the module list in `chezmoi.toml`, applies so the plists and the `MAIN.md` import render, and registers both launchd agents — idempotent, and no API calls. Before the module is on, the shell verb does not exist yet, so the first run goes through `bash scripts/bin/distill.sh --setup`. Every run — nightly, weekly, hand-run, succeeded or failed — appends a record to `.state/runs/<hostname>.jsonl` and re-renders `Runs.md`, both machines in one table, so a job that failed or harvested nothing on the Mac you weren't using is visible where you read rather than only in a launchd log. `--status` reports preflight, MAIN size against its 6 KB cap, unclassified origins, 7-day spend and the last run. `--render` rebuilds from the ledger with no API calls, `--since 7d` backfills, `--undo` reverts the vault's last distiller commit, `-n` previews without calling the model. Costs roughly $1–2 a night, bounded by `maxSpendUsd7d` in [`src/.chezmoidata/distill.toml`](../src/.chezmoidata/distill.toml). Full guide: [docs/distill.md](distill.md). |
