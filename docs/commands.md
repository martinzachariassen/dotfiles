...
| `chezdistill` | **`claudeDistiller` only.** Distil this machine's Claude Code transcripts and render the `MAIN.md` the global persona `@`-imports, so yesterday's conclusions are in context tomorrow. launchd runs it at 01:00 and `--weekly` on Sunday at 02:00; both fire on **wake** when the Mac slept through, coalescing missed firings into one run — which is why the job tracks a cursor rather than "yesterday" and so cannot lose a night. **Three destinations:** the memory tier (`MAIN.md`, `Pinned.md`, `Topics/`, `Candidates.md`) to `~/.config/claude/memory`, beside the persona that imports it; the ledger, extracts, cursor, spend and run log to `~/.local/state/chezdistill`, in a git repo with **no remote** because the extracts hold near-verbatim conversation text; and only the reports you read (`Daily/`, `Weekly/`, `Runs.md`) to `~/Documents/TheArchive/30-Claude`. **It never creates the vault** — an uncloned one looks exactly like an empty directory and would collect reports in a dead end — so when the vault, its `.obsidian` dir or `30-Claude/` is missing it says so, skips the reports, renders the memory anyway and exits 0. Nothing reaches `MAIN.md` until it has been seen in **two** distinct sessions, so one misreading can't become a global rule; `hits` is derived from the extract corpus rather than incremented, which is what makes a repeat run of an already-distilled day free. Each report opens with `## MAIN.md changes`. `--setup` is the one path allowed to create `30-Claude/`, and the way to turn the whole thing on: it verifies the vault and its `.obsidian` dir (never creating either), creates the folder behind a confirm, migrates a pre-split single-folder layout if it finds one (copying, never deleting), adds `claudeDistiller` to the module list in `chezmoi.toml`, applies so the plists and the `MAIN.md` import render, and registers both launchd agents — idempotent, and no API calls. Before the module is on, the shell verb does not exist yet, so the first run goes through `bash scripts/bin/distill.sh --setup`. Every run — nightly, weekly, hand-run, succeeded or failed — appends a record to `runs.jsonl` and re-renders `Runs.md`, so a job that failed or harvested nothing is visible where you read rather than only in a launchd log. `--status` reports where everything lives, MAIN size against its 6 KB cap, 7-day spend and the last run. `--render` rebuilds from the ledger with no API calls, `--since 7d` backfills, `--undo` reverts the state repo's last distiller commit and re-renders the memory from it, `-n` previews without calling the model. Costs roughly $1–2 a night, bounded by `maxSpendUsd7d` in [`src/.chezmoidata/distill.toml`](../src/.chezmoidata/distill.toml). Full guide: [docs/distill.md](distill.md). |
