...
| **Shell** | zsh with full XDG layout, direnv hook (auto-activates each project's devbox env on `cd`), fzf integration (`Ctrl-R` history search, `**<Tab>` completion), `zsh-completions` + `zsh-syntax-highlighting`, Claude personal wrapper, modern CLI aliases (`ls→eza`, `cat→bat`, `find→fd`). |
