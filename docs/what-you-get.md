...
| **Shell** | zsh with full XDG layout, mise activation (auto-switches each project's pinned runtimes + `JAVA_HOME` on `cd`), fzf integration (`Ctrl-R` history search, `**<Tab>` completion), zoxide smart directory jumping, Carapace completions, `zsh-completions` + `zsh-syntax-highlighting`, Claude personal config, modern CLI aliases (`ls→eza`, `cat→bat`, `find→fd`). |
...
| **Language runtimes** | **Per-project** via [mise](https://mise.jdx.dev) (a normal Homebrew formula). Global defaults live in `~/.config/mise/config.toml` (Temurin JDK 21 + 25, Node LTS); each project's repo carries its own committed `mise.toml` pinning the JDK/Node/Python versions plus an `[env]` block for project env vars. On `cd`, `mise activate` switches to the project's toolchain, sets `PATH` + `JAVA_HOME`, and unloads on the way out — no global PATH pollution. mise installs to stable, version-named paths (e.g. `~/.local/share/mise/installs/java/temurin-21/Contents/Home`), so VS Code's Java language server keeps a steady classpath. Starter `mise.toml` files for backend, Kubernetes, and Terraform projects live in [`examples/mise/`](../examples/mise/). CLIs and database servers are not mise's job — they come from Homebrew or run via Docker. |
| **Brew** | Workstation baseline + per-profile extras. Full list with one-line rationale per package in [`Brewfile`](../Brewfile). Categories: modern CLI, git productivity (`gh`, `lazygit`, `pre-commit`), `mise`, shell/editor tools, Docker Desktop, Ghostty, VS Code, 1Password, fonts, optional GUI apps, and work-profile cloud account CLIs (`az`, `gcloud`). Project Kubernetes CLIs, Terraform/OpenTofu, and DB clients/servers stay in Homebrew (or Docker); language runtimes are mise-owned. |
| **Editor (GUI)** | VS Code is installed by Homebrew. User settings are managed by chezmoi, marketplace extensions come from `vscode/extensions.txt`, and the JetBrains Kotlin extension is installed from the upstream VSIX release when the extension setup script runs. |
