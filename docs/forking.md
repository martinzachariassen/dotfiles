...
5. **Replace `.chezmoitemplates/agents/shared.md`** (the shared AI base) and **`.chezmoitemplates/claude/personal.md` (and/or `work.md`)** with your own preferences. They currently encode my style; almost certainly not yours.
6. **Update [`AGENTS.md`](../AGENTS.md)** if you restructure the repo (different Brewfile tiers, a different shell, etc.). It's the single source of truth for AI agents working in this repo — Claude Code reads it via the `CLAUDE.md` bridge, Copilot via `.github/copilot-instructions.md`. See [AI tools](ai-tools.md) for the full layering.
