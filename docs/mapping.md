...
| `dot_config/claude/CLAUDE.shared.md.tmpl` | `~/.config/claude/CLAUDE.shared.md` | Shared Claude Code base. Thin wrapper that `includeTemplate`s `.chezmoitemplates/agents/shared.md` (the single source of truth). `@import`ed by the rendered `CLAUDE.md`. Always applied. |
...
| `.chezmoitemplates/agents/shared.md` | *(not copied to $HOME)* | Single source of truth for personal AI defaults (about-me, communication, code style, environment, secrets, commits). `includeTemplate`d into `CLAUDE.shared.md`. |
...
| `dot_config/obsidian/plugins.txt` | `~/.config/obsidian/plugins.txt` | `<plugin-id>\|<github repo>[\|<tag>]` per line — source of truth for community plugins seeded into every vault. Optional `\|<tag>` pins a specific release (used for Calendar, whose `latest` is a beta with mismatched id). Consumed by `scripts/lib/obsidian-apply.sh`. |
| `dot_config/obsidian/theme.txt` | `~/.config/obsidian/theme.txt` | `<theme-name>\|<github repo>` for the canonical theme (Catppuccin). Fetched from the repo's `main` branch since theme repos don't always cut releases. |
| `dot_config/obsidian/vault-config/**` | `~/.config/obsidian/vault-config/**` | Canonical `.obsidian/` overlay — app/appearance/community-plugins/core-plugins/hotkeys JSON + per-plugin `data.json`. The apply script seeds missing files into the active vault; it never overwrites, so the Obsidian UI remains the source of truth for runtime changes. |
| `dot_config/obsidian/templates/*.md` | `~/.config/obsidian/templates/*.md` | Canonical Templater templates (daily/weekly/project/meeting/person/inbox). Seeded into `<vault>/99 Meta/_templates/` by the apply script (seed-only, like vault-config). |
| `dot_config/obsidian/Home.md` | `~/.config/obsidian/Home.md` | Dashboard the Homepage plugin opens on launch — Dataview blocks for today's daily, active projects, recent inbox, overdue tasks. Seeded once to the vault root (seed-only, so edits stick). |
...
| `scripts/lib/brew-bundle.sh` | Sourced Homebrew convergence engine: per-item install with progress + heartbeat, `bb_modules_satisfied` short-circuit, and continue-on-error. Used by `run_after_02-brew-bundle`; unit-tested by `tests/brew-bundle.bats`. See [lifecycle](lifecycle.md). | Never run directly. |
| `scripts/lib/obsidian-apply.sh` | Sourced Obsidian convergence engine: discovers the active vault from Obsidian's registry, fetches the theme + community plugins listed in `~/.config/obsidian/{theme,plugins}.txt`, and seeds canonical `.obsidian/` config + Templater templates without overwriting existing files. Used by `run_after_02d-obsidian-apply`. See [lifecycle](lifecycle.md). | Never run directly. |
| `scripts/lint-config.sh` | Validates every JSON, JSONC (VS Code), and TOML file in the source tree using `python3 -m json.tool` + `tomllib`. CI runs it in the `config-lint` job; run locally with `bash scripts/lint-config.sh` before pushing. | When editing managed JSON/TOML files; whenever CI's `config-lint` job fails. |
...
| `.chezmoiscripts/run_after_02-brew-bundle.sh.tmpl` | Runs on **every apply** and reconciles real installed state against the active Brewfile modules (state-based, not hash-gated), with a fast presence short-circuit. Continue-on-error. macOS-only. Installs `mise` (among the core formulae). Drives the engine in `scripts/lib/brew-bundle.sh`. |
| `.chezmoiscripts/run_after_02b-mise-install.sh.tmpl` | Runs on **every apply** and `mise install`s the global runtimes (java, node) from `dot_config/mise/config.toml` so the first interactive shell has them ready, with a `mise ls --missing` short-circuit. macOS-only, idempotent, doesn't fail the apply on a transient install error. |
| `.chezmoiscripts/run_after_02d-obsidian-apply.sh.tmpl` | Runs on **every apply** and seeds the active Obsidian vault with the canonical layout — Catppuccin theme, community plugins, hotkeys, app settings, Templater templates. macOS-only; gated behind `features.macApps`. Presence-only checks (freshness is `chezbump`'s job, same pattern as `02-brew-bundle`). No-op when Obsidian isn't installed or no vault is registered. Drives the engine in `scripts/lib/obsidian-apply.sh`. |
| `.chezmoiscripts/run_onchange_after_02c-cleanup-deprecated.sh.tmpl` | Removes tooling the repo no longer manages, from an explicit in-script manifest: deprecated Homebrew formulae (`direnv`, `node`) + casks (`temurin@21`, `temurin@25`, and `microsoft-outlook` — superseded by the all-in-one `microsoft-office`/M365 suite, which `conflicts_with` the standalone cask), the old out-of-band `devbox` binary, and leftover `~/.config/direnv` / devbox data dirs. Only touches named items (never a blanket sweep) and is guarded so it's a no-op on machines that never had them. The irreversible `/nix` store is removed only after an interactive y/N (skipped if there's no TTY). macOS-only. Re-fires only when the manifest changes — add future deprecations to the arrays at the top of the script. |
