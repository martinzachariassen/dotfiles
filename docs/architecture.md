...
- `.chezmoiscripts/` — auto-run hooks. Five user-visible steps per apply, in order:
  ...
  | 3 | `run_onchange_after_02-brew-bundle` | after, on Brewfile/profile/feature change | when inputs change | Layers core Brewfile + enabled workstation extras + your profile's extras (mise installs here as a normal formula). `exec </dev/tty` so sudo-requiring casks (docker-desktop, 1password) can read the password. Heartbeat every 30s during silent stretches. |
  | 2b | `run_onchange_after_02b-mise-install` | after, on global mise config change | when inputs change | Runs after brew-bundle (so `mise` is on PATH) and eagerly `mise install`s the global runtimes pinned in `~/.config/mise/config.toml` (Temurin JDK 21 + 25, Node LTS) so the first interactive shell just works. Idempotent — re-fires only when the global config changes. |
