...
- `.chezmoiscripts/` — auto-run hooks. Five user-visible steps per apply, in order:
  ...
  | 3 | `run_onchange_after_02-brew-bundle` | after, on Brewfile/profile/feature change | when inputs change | Layers core Brewfile + enabled workstation extras + your profile's extras. `exec </dev/tty` so sudo-requiring casks (docker-desktop, 1password) can read the password. Heartbeat every 30s during silent stretches. |
  ...
  | 5 | `run_once_after_04-macos-defaults` | after, once | first apply only | Runs `scripts/macos-defaults.sh`. Never re-fires automatically — re-apply edits via the `macos-defaults` zsh alias. |
