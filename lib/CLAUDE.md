...
- `lib/dot.sh` refuses a checkout path containing `"`, backtick, `$` or `\`.
  Three places bake it into generated script or `grep -F` for it:
  `core/apply.sh` and the `uninstall.sh` handoff bake, `core/doctor.sh` and
  `uninstall.sh` grep.
