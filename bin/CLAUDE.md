...
One file, `bin/dot`. Engine budget. **Five verbs, hardcoded `case`:** `apply`,
`add`, `remove`, `config`, `doctor`. A sixth goes elsewhere: a user command into
a module's `home/.local/bin/`, full removal into `uninstall.sh`.
...
- **The transcript belongs to the verbs that CHANGE the machine:** `apply`,
  `add`, `remove`, through `__transcript_start`. Not `config`, which hands the
  terminal to `$EDITOR` -- a tee in the way makes `vi` unusable. Not `doctor`,
  advertised read-only and also run as the tail of `apply`, where a second
  transcript would nest inside the first. Never under `--dry-run`.
  `uninstall.sh` removes `logs/` and must keep agreeing they are ours.
...
- **One file per run, `<timestamp>-<verb>.log`, twenty kept.** Not one per day
  ...
  leads and is fixed width, so the sort that rotates them is unaffected by the
  suffix -- but twenty bare timestamps cannot say which was the apply that
  broke something. The twenty are one budget for the directory, not per verb.
