...
# same rows and remove.sh cuts the same domains, and all three find the file
# with the same `data=` line (tests/contract.bats). A value needing a config
# setting or validation stays here.
...
# it would exit DOT_STATUS_WARN every time and `dot apply` could never reach
# "Done".
