...
# The help is generated by the dispatcher now, not a heredoc in the zshrc. The
# knobs are the part of it nothing else would surface: they are environment
# variables, so a user who does not read this list never learns they exist.
@test "chez help documents the QUIET knob" {
    grep -q 'QUIET=1' "$REPO_ROOT/core/chez.sh"
}
