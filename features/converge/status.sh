#!/usr/bin/env bash
# chez status — read-only drift report: file drift and untracked packages, in
...
main() {
    ...
    case "${1:-}" in
        -h | --help)
            echo "usage: chez status [-v|--verbose] [PATH …]"
            echo "  (no args)   plain-language summary: pending changes, local drift, untracked packages"
            echo "  PATH        raw \`chezmoi diff\` for a file (e.g. chez status ~/.zshrc)"
    ...
    status_output=$(chezmoi status --exclude scripts 2>&1) || {
        printf '%s\n' "chez status: \`chezmoi status\` failed:" >&2
        printf '%s\n' "$status_output" >&2
        return 1
    }
    if [ -z "$status_output" ]; then
        ...
        if [ -n "$pending" ]; then
            echo "── Repo → \$HOME · what \`chez apply\` would write ───────────────"
            printf '%s' "$pending"
            echo
        fi
        if [ -n "$drift" ]; then
            ...
            printf '%s' "$drift"
            echo "  ⚠ \`chez apply\` overwrites these. Keep an edit instead: chezmoi re-add ~/…"
            echo
        fi
    fi
    ...
    if command -v brew >/dev/null 2>&1; then
        ...
        if [ -n "$removals" ]; then
            ...
            echo "  reconcile (uninstall): chez mirror"
            echo
        fi
    fi

    echo "  See a file's exact lines:  chez status ~/path   ·   all:  chez status -v"
    echo "  Apply now:                 chez apply"
}
