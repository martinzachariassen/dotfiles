...
doctor_xcode() {
    ...
    if ! command -v xcode_ready >/dev/null 2>&1; then
        ...
        fail "no Xcode.app — install.sh only installs the Command Line Tools. Run: chez xcode"
    else
        pass "Xcode installed: $(xcode_app_path)"
        if xcode_selected_is_full; then
            pass "active developer dir: $(xcode-select -p)"
        else
            fail "xcode-select points at $(xcode-select -p 2>/dev/null || echo none), not Xcode.app — run: chez xcode"
        fi
        if xcode_build_works; then
            pass "xcodebuild runs: $(xcodebuild -version 2>/dev/null | head -1)"
        elif xcode_license_pending; then
            fail "Xcode licence not accepted — run: chez xcode"
        else
            fail "xcodebuild fails — run: chez xcode"
        fi
        if xcode_first_launch_done; then
            pass "first-launch components installed"
        else
            fail "Xcode first-launch components pending — run: chez xcode"
        fi
        if xcode_has_ios_sdk; then
            pass "iOS Simulator SDK present"
        else
            fail "no iOS Simulator SDK — run: chez xcode"
        fi
        ...
        if xcode_has_ios_runtime; then
            ...
            fail "no iOS simulator runtime — nothing to run an app on. Run: chez xcode"
        fi
    fi
    for tool in swiftlint swiftformat xcodegen xcode-build-server xcbeautify; do
        if command -v "$tool" >/dev/null 2>&1; then
            ...
            warn "$tool missing — run: chez apply"
        fi
    done
    ...
    if [ -f "$HOME/.config/swiftlint/config.yml" ]; then
        ...
        warn "~/.config/swiftlint/config.yml missing — run: chez apply"
    fi
    if [ -f "$HOME/.swiftformat" ]; then
        pass "~/.swiftformat present"
    else
        warn "~/.swiftformat missing — run: chez apply"
    fi
}
