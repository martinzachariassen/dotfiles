#!/usr/bin/env bash
# chezsetup — change this Mac's saved setup answers.
#
...
main() {
    ...
    if [ "$reset" -eq 1 ]; then
        ...
        if [ ! -x "$_DIR/cli.sh" ]; then
            printf 'chezsetup: %s is missing — refusing to reset run-once state\n' "$_DIR/cli.sh" >&2
            printf '           re-sync the repo first: chezup\n' >&2
            return 1
        fi
