...
distill_render_decisions() {
    local out="${1:-$DISTILL_ROOT/Decisions.md}"
    distill_have_vault || [ -n "${1:-}" ] || return 0
    mkdir -p "$(dirname "$out")"
    {
        printf '<!-- Generated by chezdistill. -->\n\n# Decisions\n\n'
        printf 'What was settled, newest first, with what it was settled against.\n'
        printf 'Every decision appears here from the first time it was made.\n\n'
        distill_derive |
...
# like from here.
distill_render_threads() {
    local out="${1:-$DISTILL_ROOT/Open threads.md}"
    distill_have_vault || [ -n "${1:-}" ] || return 0
    mkdir -p "$(dirname "$out")"
    {
        printf '<!-- Generated by chezdistill. Rebuilt every run. -->\n\n'
        printf '# Open threads\n\n'
        printf 'Left unfinished, most recently seen first. An entry disappears when it\n'
        printf 'stops being mentioned — closing it is the same as it going quiet.\n\n'
        distill_derive |
...
distill_link_topics() {
    local link="$DISTILL_ROOT/Topics" target ignore
    distill_have_vault || return 0
    target="$(distill_memory_dir)/Topics"
    [ -d "$target" ] || return 0

    if [ -L "$link" ]; then
        [ "$(readlink "$link")" = "$target" ] || ln -sfn "$target" "$link"
    elif [ -e "$link" ]; then
        # A real directory here is the pre-split layout, or a copy someone made.
        # Never replace one silently: that would delete notes.
        distill_warn "$link is a real directory, not a link — leaving it alone"
        return 0
    else
    ...
    grep -qxF "$(distill_cfg folder 30-Claude)/Topics" "$ignore" 2>/dev/null && return 0
    printf '%s/Topics\n' "$(distill_cfg folder 30-Claude)" >>"$ignore"
}
...
# point: what each note is, and the two things worth knowing about how entries get
# here and how to overrule one.
distill_render_index() {
    local out="${1:-$DISTILL_ROOT/README.md}"
    local latest
    distill_have_vault || [ -n "${1:-}" ] || return 0
    mkdir -p "$(dirname "$out")"
    latest="$(/usr/bin/find "$DISTILL_ROOT/Daily" -name '*.md' 2>/dev/null |
        sort | tail -1 | xargs -I{} basename {} .md 2>/dev/null)"
    {
        printf '<!-- Generated by chezdistill. -->\n\n# Claude, distilled\n\n'
        printf 'What past Claude Code sessions on this Mac worked out, written up each\n'
        printf 'night at 01:00. Start here.\n\n'

        printf '## Start here\n\n'
        printf -- '- [[Decisions]] — what was settled, and what it was settled against.\n'
        printf -- '- [[Open threads]] — what is still owed. Rebuilt every run.\n'
        [ -n "$latest" ] && printf -- '- [[%s]] — the most recent day.\n' "$latest"
        printf -- '- [[Runs]] — did the job run, and did it work.\n'
        printf -- '- `Topics/` — every rule with its full reasoning, grouped by subject.\n\n'

        printf '## How something gets here\n\n'
        printf 'Each night the job reads the sessions written since it last looked, and\n'
        printf 'asks a model for what is worth keeping. Those items land in `Daily/`.\n\n'
        printf 'An item that appears in **two separate sessions** is also written into\n'
        printf '`~/.config/claude/memory/MAIN.md`, which every future Claude session\n'
        printf 'loads. One conversation is not enough — that gate is what stops a single\n'
        printf 'misreading becoming a rule applied everywhere, unattended.\n\n'
        printf 'Decisions and open threads skip the gate. A decision made once is still\n'
        printf 'a decision, and you are reading these yourself.\n\n'

        printf '## Fixing one\n\n'
        printf 'Not by editing these notes — the next run rewrites them.\n\n'
        printf -- '- **A rule is wrong** → write the correct one in `%s`.\n' \
            "$(distill_tilde "$(distill_pinned_file)")"
        printf '  It goes into `MAIN.md` verbatim and is never demoted or rewritten.\n'
        printf -- '- **Last night made a mess** → `chezdistill --undo`.\n'
        printf -- '- **You changed something and want to see it** → `chezdistill --render`.\n'
        printf '  Free; no model calls.\n\n'
